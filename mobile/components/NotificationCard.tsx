...
const NotificationCard = ({ notification, onDelete }: NotificationCardProps) => {
  ...
  const getNotificationIcon = () => {
    switch (notification.type) {
      ...
      case "comment":
        return <Feather name='message-circle' size={20} color="#1D1AF2" />
      case "follow":
          ...
          <View className='flex-1'>
            <View className='flex-row items-start justify-between mb-1' >
              <View className='flex-1'>
                ...
                <Text className='text-gray-700 text-sm mb-2'>{getNotifications()}</Text>
              </View>
              <TouchableOpacity className='ml-2 p-1' onPress={handleDelete}>
                <Feather name='trash' size={16} color="#E0245E" /></TouchableOpacity>
            </View>
